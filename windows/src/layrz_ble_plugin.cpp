...
namespace layrz_ble {
  ...
  void LayrzBlePlugin::HandleMethodCall(
    ...
    if (method.compare("checkCapabilities") == 0) checkCapabilities(std::move(result));
    else if (method.compare("startScan") == 0) startScan(method_call, std::move(result));
    else if (method.compare("stopScan") == 0) stopScan(method_call, std::move(result));
  ...
  void LayrzBlePlugin::handleBleScanResult(BleScanResult& result) {
    ...
    if (filteredDeviceId.length() > 0 && result.DeviceId() != filteredDeviceId) return;
    ...
    if (it != visibleDevices.end()) {
      ...
      if (result.Rssi()) device.setRssi(result.Rssi());
      if (!result.ManufacturerData()->empty()) device.setManufacturerData(*result.ManufacturerData());
      if (!result.ServiceData()->empty()) device.setServiceData(*result.ServiceData());
    } else {
      // Add the new device
